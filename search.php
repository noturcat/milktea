...
<body>
            ...
            while($row = mysqli_fetch_assoc($result)){
                ...
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="card" style="width: 18rem;">
                        <img src="img/card-'.$catId. '.jpg" class="card-img-top" alt="image for this milktea" width="249px" height="270px">
                        <div class="card-body">
                            <h5 class="card-title"><a href="viewmilkteaList.php?catid=' . $catId . '">' . $catname . '</a></h5>
                            <a href="viewmilkteaList.php?catid=' . $catId . '" class="btn btn-primary">View All</a>
                        </div>
                    </div>
                </div>';
            }
    ...
    <div class="container my-3" id="cont">
        ...
        <?php 
            $query = $_GET["search"];
            $sql = "SELECT * FROM `milktea` WHERE MATCH(milkteaName, milkteaDesc) against('$query')"; 
            $result = mysqli_query($conn, $sql);
            while($row = mysqli_fetch_assoc($result)){
                ...
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="card" style="width: 18rem;">
                        <img src="img/milktea-'.$milkteaId. '.jpg" class="card-img-top" alt="image for this milktea" width="249px" height="270px">
                        <div class="card-body">
                            <h5 class="card-title">' . substr($milkteaName, 0, 20). '...</h5>
                            <h6 style="color: #ff0000">₱'.$milkteaPrice. '</h6>
                            <div class="row justify-content-center">';
                                if($loggedin){
                                    $quaSql = "SELECT `itemQuantity` FROM `viewcart` WHERE milkteaId = '$milkteaId' AND `userId`='$userId'";
                                    $quaresult = mysqli_query($conn, $quaSql);
                                    $quaExistRows = mysqli_num_rows($quaresult);
                                    if($quaExistRows == 0) {
                                        echo '<form action="partials/_manageCart.php" method="POST">
                                              <input type="hidden" name="itemId" value="'.$milkteaId. '">
